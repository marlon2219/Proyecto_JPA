                    ...
                    <div class="col-md-9">
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                <c:forEach var="libro" items="${librosPorEmpleado}" varStatus="status">
                                    <tr>
                                        <td>${status.count}</td> <!-- ceunta el numero de filas, otra forma de poner el id -->
                                        <td>${libro.getLibros().getTitulo()}</td>
                                        <td>${libro.getLibros().getAutor()}</td>
                                        <td>${libro.getLibros().getGenero()}</td>
                                        <td>${libro.getLibros().getPagina()}</td>
                                        <td>${libro.getLibros().getPrecio()}</td>
                                        <td>
                                            <a href="LibroServlet?accion=editar&idLibro=${libro.getLibros().getIdlibro()}" class="btn btn-outline-warning">
                                                <i class="fas fa-angle-double-right"></i> EDITAR
                                            </a>
                                            <a href="LibroServlet?accion=eliminar&idLibro=${libro.getLibros().getIdlibro()}" class="btn btn-danger btn-block" >
                                                <i class="fas fa-trash"></i> EIMINAR
                                            </a>
                                        </td>
                                    </tr>
                                </c:forEach>
                    ...
                    <div class="col-md-3 ">
                        ...
                        <div style="background-image: linear-gradient(-20deg, #fc6076 0%, #ff9a44 100%); border: none;" class="card text-center bg-success text-white mb-3">
                            <div class="card-body">
                                <h1>N° Libros</h1>
                                <h4 class="display-4">
                                    <i class="fas fa-users"></i> ${librosPorEmpleado.size()}
